...
class ListDataset(Dataset):
    def __init__(self, fname):
        self.lines = []
        lines = open('train_line_annotation.txt',
                     'r', encoding='utf8').readlines()
        self.lines += [i for i in lines if not illegal(
            i.strip('\n').split('\t')[1])]
...
class Batch:
    ...
    def __init__(self, imgs, trg_y, trg, pad=0):
        ...
        self.src_mask = torch.from_numpy(
            np.ones([imgs.size(0), 1, 96], dtype=np.bool)).to(device)
        if trg is not None:
            self.trg = trg.to(device)
            self.trg_y = trg_y.to(device)
            self.trg_mask = \
...
if __name__ == '__main__':
    ...
    dataloader = torch.utils.data.DataLoader(
        listdataset, batch_size=2, shuffle=False, num_workers=0)
