...
class EncoderDecoder(nn.Module):
    ...
    def __init__(self, encoder, decoder, src_embed, tgt_embed, generator, vocab, d_model):
    ...
    def forward(self, src, tgt, src_mask, tgt_mask):
        t = self.encode(src, src_mask)
        return self.decode(t, src_mask, tgt, tgt_mask)

    def encode(self, src, src_mask):

        # b = self.src_embed(src.long()) * math.sqrt(self.d_model)
        return self.encoder(self.src_embed(src), src_mask)
        # return self.encoder(b, src_mask)
...
class Embeddings(nn.Module):
    def __init__(self, d_model, vocab):
        ...
        self.lut = nn.Embedding(vocab, d_model).to(torch.device('cuda'))
        self.d_model = d_model
    def forward(self, x):
...
def make_model(tgt_vocab, N=4,
    ...
    model = EncoderDecoder(
        ...
        Decoder(DecoderLayer(d_model, c(attn), c(attn),
                             c(ff), dropout), N),
        nn.Sequential(FeatureExtractor(d_model, resnet,
                                       'layer3', d_feature), c(position)),
        nn.Sequential(Embeddings(d_model, tgt_vocab), c(position)),
        Generator(d_model, tgt_vocab), tgt_vocab, d_model)
